...
class TCPAResult:
    ...
    cpa_time: pd.Timestamp | None = None
    cpa_position_1: tuple[float, float] | None = None  # (x, y)
    cpa_position_2: tuple[float, float] | None = None  # (x, y)
...
class TCPADCPACalculator:
    ...
    def calculate_tcpa_dcpa_matrix(
        self, df: pd.DataFrame, timestamp: pd.Timestamp
    ...
    def calculate_risk_graph(
        self, df: pd.DataFrame, timestamp: pd.Timestamp, decay_factor: float = 0.1
    ) -> np.ndarray:
        """
        ...
        """
        tcpa_matrix, dcpa_matrix, vessels = self.calculate_tcpa_dcpa_matrix(
            df, timestamp
        )
    ...
    def calculate_batch_tcpa_dcpa(
        self, df: pd.DataFrame, time_window_minutes: float = 5.0
    ) -> list[TCPAResult]:
        """
            ...
            for i, vessel_1 in enumerate(vessels):
                for vessel_2 in vessels[i + 1 :]:
                    ...
                    if len(v1_data) > 0 and len(v2_data) > 0:
                        ...
                        # Calculate TCPA/DCPA
                        result = self._calculate_tcpa_dcpa_pair(
                            v1_pos, v2_pos, window_time
                        )
    ...
    def _prepare_vessel_positions(
        self, df: pd.DataFrame, reference_time: pd.Timestamp
    ) -> dict[int, VesselPosition]:
    ...
    def _calculate_tcpa_dcpa_vectorized(
        self, pos1: VesselPosition, pos2: VesselPosition
    ) -> tuple[float, float]:
        """
    ...
    def _calculate_tcpa_dcpa_pair(
        self, v1_row: pd.Series, v2_row: pd.Series, reference_time: pd.Timestamp
    ) -> TCPAResult | None:
        ...
        if (
            v1_row.get("speed", 0) < self.min_speed_threshold
            and v2_row.get("speed", 0) < self.min_speed_threshold
        ):
            return None
        ...
        x1, y1 = self._project_to_local(
            v1_row["latitude"], v1_row["longitude"], center_lat, center_lon
        )
        x2, y2 = self._project_to_local(
            v2_row["latitude"], v2_row["longitude"], center_lat, center_lon
        )
        ...
        pos1 = VesselPosition(
            mmsi=v1_row["mmsi"],
            timestamp=v1_row["timestamp"],
            latitude=v1_row["latitude"],
            longitude=v1_row["longitude"],
            speed=v1_row.get("speed", 0.0),
            course=v1_row.get("course", 0.0),
            x=x1,
            y=y1,
            vx=vx1,
            vy=vy1,
        ...
        pos2 = VesselPosition(
            mmsi=v2_row["mmsi"],
            timestamp=v2_row["timestamp"],
            latitude=v2_row["latitude"],
            longitude=v2_row["longitude"],
            speed=v2_row.get("speed", 0.0),
            course=v2_row.get("course", 0.0),
            x=x2,
            y=y2,
            vx=vx2,
            vy=vy2,
    ...
    def _calculate_risk_score(self, tcpa: float, dcpa: float) -> float:
        ...
        distance_risk = max(
            0, (self.risk_distance_threshold - dcpa) / self.risk_distance_threshold
        )
    ...
    def get_high_risk_pairs(
        self, results: list[TCPAResult], risk_threshold: float = 0.5
    ) -> list[TCPAResult]:
    ...
    def get_statistics(self, results: list[TCPAResult]) -> dict[str, Any]:
        ...
        stats = {
            ...
            "medium_risk_count": len(
                [r for r in results if 0.3 <= r.risk_level <= 0.7]
            ),
