...
class VesselState:
    ...
    navigation_state: Optional[int] = None
...
class SceneAggregator:
    ...
    def aggregate_scenes(self, df: pd.DataFrame) -> List[Scene]:
        ...
        for window_start, window_states in time_windows.items():
            ...
            if len(set(state.mmsi for state in window_states)) < self.min_vessels_per_scene:
                continue
        ...
        enriched_scenes = self._enrich_scenes_with_interactions(merged_scenes, interaction_events)
    ...
    def _create_time_windows(self, vessel_states: List[VesselState]) -> Dict[pd.Timestamp, List[VesselState]]:
    ...
    def _identify_spatial_clusters(self, vessel_states: List[VesselState]) -> List[List[VesselState]]:
        ...
        while remaining_states:
            ...
            if len(set(state.mmsi for state in cluster)) >= self.min_vessels_per_scene:
    ...
    def _classify_scene_type(self, vessel_states: List[VesselState]) -> SceneType:
        ...
        vessel_count = len(set(state.mmsi for state in vessel_states))
    ...
    def _classify_port_scene(self, vessel_states: List[VesselState], center_lat: float, center_lon: float) -> SceneType:
    ...
    def _create_scene(self, scene_type: SceneType, vessel_states: List[VesselState], window_start: pd.Timestamp) -> Optional[Scene]:
        ...
        center_lat = np.mean([state.latitude for state in vessel_states])
        center_lon = np.mean([state.longitude for state in vessel_states])
        vessels = list(set(state.mmsi for state in vessel_states))
        ...
        # Generate scene ID
        scene_id = f"{scene_type.value}_{window_start.strftime('%Y%m%d_%H%M')}_{len(vessels)}v"
        ...
        properties = {
            ...
            'vessel_types': list(set(state.vessel_type for state in vessel_states if state.vessel_type)),
            'navigation_states': list(set(state.navigation_state for state in vessel_states if state.navigation_state)),
    ...
    def _merge_temporal_scenes(self, scenes: List[Scene]) -> List[Scene]:
        ...
        for next_scene in scenes[1:]:
            ...
            if (next_scene.scene_type == current_scene.scene_type and
                next_scene.start_time - current_scene.end_time <= self.min_scene_duration and
                self._scenes_spatially_overlap(current_scene, next_scene)):
    ...
    def _merge_two_scenes(self, scene1: Scene, scene2: Scene) -> Scene:
        ...
        new_center_lat = (scene1.center_lat * w1 + scene2.center_lat * w2) / total_weight
        new_center_lon = (scene1.center_lon * w1 + scene2.center_lon * w2) / total_weight
        ...
        combined_properties.update({
            ...
            'density': len(combined_vessels) / (np.pi * (self.scene_radius_km ** 2))
        })
        ...
        for window_time, window_df in time_windows:
            ...
            for i, vessel1 in enumerate(vessels):
                for vessel2 in vessels[i+1:]:
                    ...
                    interaction_event = self._analyze_vessel_interaction(v1_data, v2_data, window_time)
    ...
    def _analyze_vessel_interaction(self, v1_data: pd.DataFrame, v2_data: pd.DataFrame, window_time: pd.Timestamp) -> Optional[InteractionEvent]:
    ...
    def _classify_interaction_type(self, v1_data: pd.DataFrame, v2_data: pd.DataFrame) -> InteractionType:
    ...
    def _calculate_risk_level(self, min_distance: float, v1_data: pd.DataFrame, v2_data: pd.DataFrame) -> float:
    ...
    def _enrich_scenes_with_interactions(self, scenes: List[Scene], interaction_events: List[InteractionEvent]) -> List[Scene]:
        """Add interaction information to scenes"""
        enriched_scenes = []
        ...
        for scene in scenes:
            ...
            for interaction in interaction_events:
                ...
                if (interaction.start_time <= scene.end_time and 
                    interaction.end_time >= scene.start_time):
                    
                    # Check if vessels are in the scene
                    if (interaction.vessel_1 in scene.vessels or 
                        interaction.vessel_2 in scene.vessels):
    ...
    def get_scene_statistics(self, scenes: List[Scene]) -> Dict[str, Any]:
        ...
        durations = [scene.duration.total_seconds() / 3600.0 for scene in scenes]  # hours
