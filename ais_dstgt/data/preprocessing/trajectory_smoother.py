...
class EnhancedTrajectorySmootherRTS:
    ...
    def smooth_trajectory(self, df: pd.DataFrame) -> SmoothingResult:
        ...
        cleaned_data, outliers_removed = self._detect_and_remove_outliers(
            projected_data
        )
        ...
        logger.info(
            f"Smoothed trajectory for MMSI {result.mmsi}: "
    ...
    def _detect_and_remove_outliers(self, df: pd.DataFrame) -> tuple[pd.DataFrame, int]:
        ...
        df_clean["calc_speed"] = (
            df_clean["distance"] / df_clean["dt"] * 1.94384
        )  # m/s to knots

        # Speed-based outlier detection
        speed_outliers = (df_clean["calc_speed"] > self.max_speed_threshold) | (
    ...
    def _interpolate_gap(
        self, point1: pd.Series, point2: pd.Series, n_points: int
    ) -> list[dict]:
        ...
        for i in range(1, n_points + 1):
            ...
            interp_lat = point1["latitude"] + alpha * (
                point2["latitude"] - point1["latitude"]
            )
            interp_lon = point1["longitude"] + alpha * (
            ...
            interp_speed = point1.get("speed", 0) + alpha * (
                point2.get("speed", 0) - point1.get("speed", 0)
            )
            interp_course = self._interpolate_course(
                point1.get("course", 0), point2.get("course", 0), alpha
            )
    ...
    def _interpolate_course(
        self, course1: float, course2: float, alpha: float
    ) -> float:
    ...
    def _apply_rts_smoothing(
        ...
        for i in range(n_points - 2, -1, -1):
            ...
            if i < n_points - 1:
                self.dt = (
                    df.iloc[i + 1]["timestamp"] - df.iloc[i]["timestamp"]
                ).total_seconds()
            ...
            # Smoothed estimates
            smoothed_states[i] = smoothed_states[i] + A @ (
                smoothed_states[i + 1] - x_pred
            )
    ...
    def _convert_to_smoothed_points(
        self,
    ...
    def _unproject_point(
        self, x: float, y: float, center_lat: float, center_lon: float
    ) -> tuple[float, float]:
    ...
    def _calculate_point_quality(
        self, unc_x: float, unc_y: float, unc_vx: float, unc_vy: float
    ) -> float:
        ...
        pos_quality = 1.0 - min(
            1.0, np.sqrt(unc_x**2 + unc_y**2) / max_pos_uncertainty
        )
        vel_quality = 1.0 - min(
            1.0, np.sqrt(unc_vx**2 + unc_vy**2) / max_vel_uncertainty
        )
    ...
    def _calculate_quality_metrics(
        ...
        # Data completeness
        interpolated_ratio = sum(1 for p in smoothed_points if p.is_interpolated) / len(
            smoothed_points
        )

        metrics = {
            "avg_position_error_m": np.mean(position_errors)
            if position_errors
            else 0.0,
            ...
            "outlier_ratio": outliers_removed / len(original_df)
            if len(original_df) > 0
            else 0.0,
            "data_reduction_ratio": (len(original_df) - len(smoothed_points))
            / len(original_df)
            if len(original_df) > 0
            else 0.0,
        }
    ...
    def smooth_multiple_trajectories(
        self, df: pd.DataFrame
    ) -> dict[int, SmoothingResult]:
        """
    ...
    def export_smoothed_trajectories(
        self, results: dict[int, SmoothingResult]
    ) -> pd.DataFrame:
