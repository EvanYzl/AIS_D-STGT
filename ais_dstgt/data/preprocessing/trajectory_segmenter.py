...
class TrajectorySegmenter:
    ...
    def __init__(self, 
                 ...
                 maneuvering_course_change_threshold: float = 45.0):  # degrees
    ...
    def segment_trajectory(self, df: pd.DataFrame) -> List[TrajectorySegment]:
        ...
        all_segments = (gap_segments + port_segments + anchoring_segments + 
                       maneuvering_segments + behavior_segments)
    ...
    def _get_port_name(self, row) -> Optional[str]:
            ...
            if gap_idx > start_idx:
                segment = TrajectorySegment(
                    ...
                    properties={
                        ...
                        'time_gap': df.iloc[gap_idx]['time_diff'] if gap_idx < len(df) else 0,
                        'distance_gap': df.iloc[gap_idx]['distance'] if gap_idx < len(df) else 0
                    }
                )
    ...
    def _detect_anchoring_periods(self, df: pd.DataFrame) -> List[TrajectorySegment]:
        ...
        if 'navigation_state' in df.columns:
            anchored_state = df['navigation_state'].isin([
                NavigationState.AT_ANCHOR.value,
                NavigationState.MOORED.value
            ])
    ...
    def _detect_maneuvering_periods(self, df: pd.DataFrame) -> List[TrajectorySegment]:
        ...
        high_course_change = df['course_change'] > self.maneuvering_course_change_threshold
        ...
        for i in range(0, len(change_indices)-1, 2):
            ...
            if end_idx > start_idx:
                segment = TrajectorySegment(
                    ...
                    properties={
                        'avg_course_change': df.iloc[start_idx:end_idx+1]['course_change'].mean(),
                        'avg_speed': df.iloc[start_idx:end_idx+1]['speed'].mean()
                    }
                )
                segments.append(segment)
    ...
    def _merge_and_fill_segments(self, df: pd.DataFrame, 
                                segments: List[TrajectorySegment]) -> List[TrajectorySegment]:
        """Merge overlapping segments and fill gaps"""
        if not segments:
            return []
        ...
        for next_segment in segments[1:]:
            ...
            if (next_segment.start_time <= current_segment.end_time or
                next_segment.start_idx <= current_segment.end_idx + 1):
                ...
                if segment_priority.get(next_segment.segment_type, 0) > segment_priority.get(current_segment.segment_type, 0):
    ...
    def segment_trajectories(self, df: pd.DataFrame) -> Dict[int, List[TrajectorySegment]]:
        """
        ...
        for mmsi in df['mmsi'].unique():
            ...
            logger.info(f"Segmented trajectory for MMSI {mmsi}: {len(segments)} segments")
        
        return segments_by_mmsi
    
    def get_segment_statistics(self, segments: List[TrajectorySegment]) -> Dict[str, Any]:
