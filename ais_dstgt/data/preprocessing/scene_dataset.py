...
class DatasetConfig:
    ...
    filter_scene_types: Optional[List[str]] = None
    
    def __post_init__(self):
        if self.node_feature_names is None:
            self.node_feature_names = [
                'speed', 'course', 'acceleration', 'course_change',
                'distance_to_center', 'relative_bearing'
            ]
...
class SceneDataset(Dataset):
    ...
    def __init__(self, 
                 scenes_data: Union[List[Dict], str, Path],
                 vessel_trajectories: Union[pd.DataFrame, str, Path],
    ...
    def _load_scenes_data(self, scenes_data: Union[List[Dict], str, Path]) -> List[Dict]:
    ...
    def _load_trajectories(self, trajectories: Union[pd.DataFrame, str, Path]) -> pd.DataFrame:
    ...
    def _filter_scenes(self) -> List[Dict]:
        ...
        for scene in self.scenes_data:
            ...
            if not (self.config.min_vessel_count <= vessel_count <= self.config.max_vessel_count):
                continue
    ...
    def _extract_scene_features(self, scene_data: Dict) -> SceneFeatures:
        ...
        node_features, node_positions, node_velocities, node_types = self._extract_node_features(
            scene_trajectories, scene_timestamp
        )
    ...
    def _get_scene_trajectories(self, vessels: List[int], timestamp: pd.Timestamp) -> pd.DataFrame:
    ...
    def _extract_node_features(self, trajectories: pd.DataFrame, 
                              timestamp: pd.Timestamp) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for i, vessel_mmsi in enumerate(vessels):
            ...
            for feature_name in self.config.node_feature_names:
                if feature_name == 'speed':
                    ...
                    features.append(self._calculate_acceleration(vessel_data, closest_idx))
                elif feature_name == 'course_change':
                    features.append(self._calculate_course_change(vessel_data, closest_idx))
                elif feature_name == 'distance_to_center':
                    features.append(self._calculate_distance_to_center(
                        vessel_row, scene_center_lat, scene_center_lon
                    ))
                elif feature_name == 'relative_bearing':
                    features.append(self._calculate_relative_bearing(
                        vessel_row, scene_center_lat, scene_center_lon
                    ))
                else:
            ...
            x, y = self._project_to_local(
                vessel_row['latitude'], vessel_row['longitude'],
    ...
    def _extract_edge_features(self, positions: torch.Tensor, velocities: torch.Tensor,
                              trajectories: pd.DataFrame) -> Tuple[torch.Tensor, torch.Tensor]:
            ...
            for j in range(i + 1, n_vessels):
                ...
                if distance <= self.config.edge_distance_threshold:
                    ...
                    if self.config.include_edge_features:
                        ...
                        rel_pos = pos_j - pos_i
                        rel_vel = vel_j - vel_i
                        approach_rate = -torch.dot(rel_pos, rel_vel).item() / (distance + 1e-6)
                        
                        # Bearing
                        bearing = torch.atan2(rel_pos[0], rel_pos[1]).item()
                        
                        edge_feat = [distance, rel_velocity, approach_rate, bearing]
                        edge_features_list.extend([edge_feat, edge_feat])  # Same for both directions
                    else:
        ...
        if not edge_list:
            ...
            edge_features = torch.zeros(0, 1 if not self.config.include_edge_features else 4)
        else:
    ...
    def _extract_graph_features(self, scene_data: Dict, trajectories: pd.DataFrame) -> torch.Tensor:
    ...
    def _extract_temporal_features(self, vessels: List[int], 
                                  timestamp: pd.Timestamp) -> Tuple[torch.Tensor, torch.Tensor]:
        """Extract historical and future features"""
        n_vessels = len(vessels)
        n_features = 4  # x, y, vx, vy
        
        history_features = torch.zeros(n_vessels, self.config.history_length, n_features)
        ...
        for i, vessel_mmsi in enumerate(vessels):
            ...
            # Extract future
            future_end = min(len(vessel_data), ref_position + self.config.future_length + 1)
    ...
    def _calculate_acceleration(self, vessel_data: pd.DataFrame, current_idx: int) -> float:
        ...
        current_speed = vessel_data.loc[current_idx].get('speed', 0.0) * 0.514444  # knots to m/s
        prev_speed = vessel_data.iloc[pos-1].get('speed', 0.0) * 0.514444
        
        time_diff = (vessel_data.loc[current_idx]['timestamp'] - 
                    vessel_data.iloc[pos-1]['timestamp']).total_seconds()
    ...
    def _calculate_course_change(self, vessel_data: pd.DataFrame, current_idx: int) -> float:
    ...
    def _calculate_distance_to_center(self, vessel_row: pd.Series, 
    ...
    def _calculate_relative_bearing(self, vessel_row: pd.Series,
                                   center_lat: float, center_lon: float) -> float:
    ...
    def _compute_feature_statistics(self):
        ...
        sample_indices = np.random.choice(len(self.valid_scenes), sample_size, replace=False)
    ...
    def _load_cached_features(self, scene_id: str) -> Optional[SceneFeatures]:
...
def collate_scene_batch(batch: List[SceneFeatures]) -> Dict[str, torch.Tensor]:
    ...
    if edge_indices_list:
        ...
        batched_data['edge_features'] = torch.zeros(0, edge_features_list[0].shape[1] if edge_features_list else 1)
    ...
    if history_features_list:
        ...
        padded_future = torch.zeros(len(batch), max_nodes, future_features_list[0].shape[1], n_features)
        
        for i, (hist, fut) in enumerate(zip(history_features_list, future_features_list)):
...
def create_scene_dataloader(dataset: SceneDataset,
                           batch_size: int = 32,
                           shuffle: bool = True,
                           num_workers: int = 0,
                           **kwargs) -> DataLoader:
